...
let tpics = document.querySelectorAll(".tpic");
...
// let topic = topicSub[1].textContent.trim().replace(/^\s+|\s+$/gm, "");
// topic = topic.replace(topic[0], "").trim();
// console.log(topic);

// console.log(tpics);

tpics.forEach((el, i) => {
  let topic = topicSub[i].textContent.trim().replace(/^\s+|\s+$/gm, "");
  topic = topic.replace(topic[0], "").trim();
  tpics[i].textContent = topic;
});
